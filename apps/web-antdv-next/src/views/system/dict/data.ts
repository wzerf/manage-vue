  ...
  return [
    ...
    },
    {
      ...
      dependencies: {
        ...
        resolve: ({ values }) => ({
          show: values.usePresetStyle !== false,
        }),
      },
      componentProps: {
...
export function useDataSearchSchema(): VbenFormProps['schema'] {
  return [
    ...
    },
    {
      ...
      // dependencies.resolve({ show: false }) 用 v-show 把 FormItem 隐藏（FormField
      // 仍然挂载，vee-validate 才能感知到字段；不能直接用 hide: true / if: false，
      // 否则 v-if 摘掉 FormField，field 不会被 useField 注册，form.values 里就没这个
